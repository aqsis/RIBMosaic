...
# #############################################################################
#
# Property module providing declaration, creation and destruction of properties.
#
...
primvar_desc = "Specify what RenderMan primitive type to export or use auto select"
...
class RibmosaicPassProps(bpy.types.PropertyGroup):
    ...
    pass_display_file = bpy.props.StringProperty(name="Display File",
                ...
                maxlen=512,
                default="Renders/@[EVAL:.current_frame:####]@@[EVAL:.pass_layer:]@.tif")
    ...
    pass_texturemem = bpy.props.IntProperty(name="Texture Mem",
                description="Maximum texture space in memory in kb, 0 disables",
                default=0,
                min=0,
                soft_min=0,
                soft_max=262144)
    ...
    pass_filter = bpy.props.StringProperty(name="Pixel Filter",
                description="Pixel filter (box, sinc, triangle, gaussian, ect)",
                maxlen=256,
                default="")
    ...
    pass_passid = bpy.props.IntProperty(name="Pass ID",
                description="Use to uniquely identify passes from panel scripts",
                default=0,
                min=0,
                soft_min=0,
                soft_max=65535)
    ...
    pass_seq_width = bpy.props.IntProperty(name="Sequence Width",
                description="Number of frames in each sequence pass, 0 disables",
                default=0,
                min=0,
                max=300000,
                soft_min=0,
    ...
    pass_layerfilter = bpy.props.StringProperty(name="Render Layer",
                description="Specify a render layer to filter this passes objects "
                            "and assign render output to Blender's render result",
                maxlen=32,
                default="")
    
    pass_panelfilter = bpy.props.StringProperty(name="Panel Filter",
                description="Specify a Python expression to filter all pipeline "
                            "panels in this pass (0/False=disable, 1/True=enable)",
                maxlen=512,
                default="")
...
class RibmosaicPassGroup(bpy.types.PropertyGroup):
    ...
    collection = bpy.props.CollectionProperty(type=RibmosaicPassProps,
                name="List of passes",
                description="")
    
    active_index = bpy.props.IntProperty(name="Index of the active pass",
                description="",
                default=0,
                min=0,
                max=65535)
...
def create_props():
    ...
    try:
        ...
        try:
            bpy.types.unregister(rm.rm_operator.WM_OT_ribmosaic_library_addpanel)
            bpy.types.register(rm.rm_operator.WM_OT_ribmosaic_library_addpanel)
        except:
            pass
    ...
    for p in exporter_props:
        ...
        exec(data + ".ribmosaic_rib_archive = bpy.props.EnumProperty("
             "name=\"RIB Archive\","
             "description=\"Specify archive and linking method for " + p[1] + " RIBs\","
             "items=p[2],"
             "default=p[3])")
    ...
    for p in geometry_props:
        data = "bpy.types." + p
        seq = [("range", p + "'s range of visibility from last level to next", 25.0),
               ("trans", "Transitional distance between this and next level", 5.0)]
        
        exec(data + ".ribmosaic_lod = bpy.props.IntProperty("
             "name=\"Levels of Detail\","
             "description=\"Use LOD by defining what data-blocks show at what ranges\","
             "default=0,"
             "min=0,"
             "max=8,"
             "soft_min=0,"
    ...
    bpy.types.WindowManager.ribmosaic_pipeline_search = bpy.props.StringProperty(
    ...
    bpy.types.WindowManager.ribmosaic_preview_shading = bpy.props.FloatProperty(
                name="Shading",
    ...
    bpy.types.WindowManager.ribmosaic_preview_compile = bpy.props.BoolProperty(
                name="Compile Shaders",
                description="Compile shaders for preview render",
                default=True)
    
    bpy.types.WindowManager.ribmosaic_preview_optimize = bpy.props.BoolProperty(
    ...
    bpy.types.Scene.ribmosaic_interactive = bpy.props.BoolProperty(
                ...
                description="Exports and renders active render pass as scene changes "
                            "(renderer must support progressive rendering)",
                default=False)
    ...
    bpy.types.Scene.ribmosaic_activeobj = bpy.props.BoolProperty(
                name="Active Object",
                description="Only export selected object (and any data in object)",
                default=False)
    
    bpy.types.Scene.ribmosaic_purgerib = bpy.props.BoolProperty(
                name="Purge RIB",
                description="Purge RIB passes (existing RIB reused if disabled)",
                default=True)
    
    bpy.types.Scene.ribmosaic_exportrib = bpy.props.BoolProperty(
                name="Export RIB",
                description="Export RIB passes (existing RIB reused if disabled)",
                default=True)
    ...
    bpy.types.Scene.ribmosaic_purgeshd = bpy.props.BoolProperty(
                name="Purge Shaders",
                description="Purge shaders (existing shaders reused if disabled)",
                default=True)
    
    bpy.types.Scene.ribmosaic_compileshd = bpy.props.BoolProperty(
                name="Compile Shaders",
                description="Compile shaders (existing shaders reused if disabled)",
                default=True)
    
    bpy.types.Scene.ribmosaic_purgetex = bpy.props.BoolProperty(
                name="Purge Textures",
                description="Purge textures (existing textures reused if disabled)",
                default=True)
    
    bpy.types.Scene.ribmosaic_optimizetex = bpy.props.BoolProperty(
                name="Optimize Textures",
                description="Optimize textures (existing textures reused if disabled)",
                default=True)
    ...
    bpy.types.Scene.ribmosaic_compressrib = bpy.props.BoolProperty(
                ...
                description="Compress RIB export (gzip archives with .rib extensions)",
                default=False)
    
    bpy.types.Scene.ribmosaic_use_frame = bpy.props.BoolProperty(
                name="Export RiFrameBegin",
                description="Export RiFrameBegin/End blocks (compatibility switch)",
                default=True)
    
    bpy.types.Scene.ribmosaic_use_world = bpy.props.BoolProperty(
                name="Export RiWorldBegin",
                description="Export RiWorldBegin/End blocks (compatibility switch)",
                default=True)
    ...
    bpy.types.Scene.ribmosaic_export_path = bpy.props.StringProperty(
                name="Export Path",
                description="Path for export and execution (created if not present)",
    ...
    bpy.types.Scene.ribmosaic_object_archives = bpy.props.EnumProperty(
            ...
            description="Specify archive and linking method for all object RIBs",
    ...
    bpy.types.Scene.ribmosaic_material_archives = bpy.props.EnumProperty(
            ...
            description="Specify archive and linking method for all material RIBs",
    ...
    bpy.types.Mesh.ribmosaic_n_class = bpy.props.EnumProperty(
                name="N Class",
                description="Specify what N primitive variable class to export",
                items=primvar_class,
                default='FACEVARYING')
    
    bpy.types.Mesh.ribmosaic_cs_class = bpy.props.EnumProperty(
                name="Cs Class",
                description="Specify what Cs primitive variable class to export",
                items=primvar_class,
                default='FACEVARYING')
    
    bpy.types.Mesh.ribmosaic_st_class = bpy.props.EnumProperty(
                name="st Class",
                description="Specify what st primitive variable class to export",
                items=primvar_class,
                default='FACEVARYING')
    ...
    bpy.types.Mesh.ribmosaic_cs_export = bpy.props.BoolProperty(
                ...
                description="Export active vertex color as Cs primitive variables",
                default=True)
    ...
    bpy.types.Camera.ribmosaic_relative_detail = bpy.props.FloatProperty(
                ...
                description="Scale the calculated LOD for all objects in scene",
                precision=3,
                default=1.0,
                min=0.0,
                soft_min=0.0,
    ...
    bpy.types.Material.ribmosaic_disp_pad = bpy.props.FloatProperty(
                name="Displace Bound",
                description="Adjust bounding box for displacements, 0 disables",
                precision=3,
                default=0.0,
                min=0.0,
                max=64.0,
    ...
    bpy.types.Material.ribmosaic_disp_coor = bpy.props.EnumProperty(
                ...
                description="Coordinate system the displacement bound is measured in",
                items=[('WORLD', "World", ""),
                       ('OBJECT', "Object", ""),
                       ('CAMERA', "Camera", ""),
                       ('SCREEN', "Screen", ""),
