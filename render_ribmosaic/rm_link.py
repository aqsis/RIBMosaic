...
class PipelineLink(rm_context.ExportContext):
    ...
    Links insert text from one XML element into the text or attribute of another
    element. Link format is @[token:xmlpath(paras):option*depth]@ where token
    determines the link type, xmlpath points to the data, paras can be named
    ...
    EXEC = XML path to script or "." + export context method (both use parameters)
    ...
    *# = Sets the recursion limit for current link
    ...
    def _resolve_path(self):
        ...
        # Split out segments
        text = ""
        try: token = link.split(":", 1)[0]
        except: token = ""
        try: path = link.split(":", 1)[1].rsplit(":", 1)[0]
        except: path = ""
        try: option = link.rsplit(":", 1)[1].split("*")[0]
        except: option = ""
        try: self.recursion_max = int(link.rsplit(":", 1)[1].split("*")[1])
        except: pass
        
        if token not in self._link_tokens:
            rm.RibmosaicInfo("Available token types: " + ", ".join(self._link_tokens))
        ...
        if token != 'EVAL': # Make sure this has a xmlpath
            ...
            if not path.startswith(".") or token == 'ATTR': # Ignore EXEC commands
                ...
                for i, e in enumerate(elements[:4]):
                    if not e: # resolve relative path
                        ...
                        if i == 0: self.context_pipeline = e
                        elif i == 1: self.context_category = e
                        elif i == 2: self.context_panel = e
        ...
        try:
            if token == 'STXT' or token == 'SXML':
                ...
                text = export + os.sep.join(paths['SHD']) + os.sep + path + os.sep
            elif token == 'SLIB':
                ...
                if path.startswith(".") or "properties" in path:
                    ...
                    if token == 'DATA':
                        if self.pointer_datablock:
                            ...
                            rm.RibmosaicInfo("No datablock specified in ExportContext")
                            text = ""
                    else:
                        text = prop
                else:
                    rm.RibmosaicInfo("DATA and PROP links only work for panel properties")
                    text = ""
            elif token == 'ATTR':
                ...
                try:
                    if path.startswith("."): # Is this is an exporter method
                        ...
                        exec(rm.pipeline_manager.get_text(self, path), globals(), local)
        ...
        if option:
            if '#' in option: # Return leading zeros
                ...
                if text.isdigit():
                    ...
                    text = option.replace("#", "0")[len(text):len(option)] + text
                else:
                    rm.RibmosaicInfo("Link must return numbers to use # option")
            elif option == 'RIB': # Parse text for RIB output
                if token == 'STXT' or token == 'SXML' or token == 'SLIB' or \
                   (token == 'EVAL' and path.startswith(".") and "path" in path):
                    export = rm.export_manager.export_directory
