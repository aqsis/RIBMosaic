...
class PipelineManager():
    ...
    _pipeline_elements = {
        ...
        'pipeline':{
            ...
            'attributes':{
                ...
                'filepath':{
                    ...
                    'options':[""],
                    'default':0,
                    'comment':"Internal use only (tracks text datablock name)"},
                'filter':{
                    'method':"ADD",
                    'options':[""],
                    'default':0,
                ...
                'library':{
                    ...
                    'options':[""],
                    'default':0,
                    'comment':"Filepath to folder containing RenderMan shaders\n"
                              "Once specified pipeline maintains all shaders\n"
                              "in library and panels can be added and updated"},
                'version':{
                    'method':"SET",
                    'options':[""],
                    'default':0,
                    'comment':"RIB Mosaic version the pipeline was created with,\n"
                              "a warning is issued when run on a newer version"}}},
        # Python script elements
        'python_scripts':{
            'comment':"Contains elements defining Python scripts\n"
                      "(usually executed by links however can also just be text)",
            'text':"\n",
            'tail':"\n\n",
            'sub_elements':[],
            'user_elements':[
            ...
            'attributes':{}},
        'python_script':{
            'comment':"Element's text used as Python code for link execution\n"
                      "(text's leading and trailing whitespace stripped)",
            'text':"\n",
            'tail':"\n",
            'sub_elements':[],
            ...
            'attributes':{
                'description':{
                    ...
                    'options':[""],
                    'default':0,
                    'comment':"Description about element's usage (used in UI)"},
                'filepath':{
                    'method':"FILE",
                    'options':[""],
                    'default':0,
                    'comment':"If set, the external file to update/save to"}}},
        ...
        'shader_sources':{
            ...
            'attributes':{}},
        'shader_source':{
            'comment':"Element's text exported and compiled as shader RSL code\n"
                      "(text's leading and trailing whitespace stripped)",
            'text':"\n",
            'tail':"\n",
            'sub_elements':[],
            ...
            'attributes':{
                ...
                'windows':{
                    ...
                    'options':["MATERIAL",
                               ...
                               "LAMP"],
                    'default':0,
                    'comment':"Assign panel to Blender \"Properties\" windows\n"
            ...
            'attributes':{
                ...
                'type':{
                    ...
                    'options':["OPTIMIZE",
                               ...
                               "POSTRENDER"],
                    'default':3,
                    'comment':"Determine type and order of command execution"},
                'execute':{
                    'method':"SET",
                    'options':["True", "False"],
                    'default':0,
                    'comment':"Execute this commend script after export\n"
                              "(TIP: use link here to trigger scripts for farms)"},
                'extension':{
                    'method':"SET",
                    'options':[".sh.bat"],
                    'default':0,
                    'comment':"File extension used for exported command script"},
                'description':{
                    'method':"SET",
                    'options':[""],
                    'default':0,
            ...
            'attributes':{
                'target':{
                    ...
                    'comment':"Filepath of file/s to target during export\n"
                              ...
                              "Use the *.ext wildcard to target multiple files\n"
                              "\n"
                              "The element's text is exported for each match\n"
                              "Context's .target_path and .target_name are set"}}},
        'read':{
            ...
            'attributes':{
                'target':{
                    ...
                    'comment':"Filepath of file/s to target during export\n"
                              ...
                              "Use the *.ext wildcard to target multiple files\n"
                              "\n"
                              "The regexes are applied to each matching target"}}},
        'regex':{
            'comment':"String replacement regular expression to apply to target",
            'text':None,
            'tail':"\n",
            'sub_elements':[],
            'user_elements':[],
            'attributes':{
                'matches':{
                    ...
                    'options':["0"],
                    'default':0,
                    'comment':"Number of matches per archive (0 = all matches)"},
                'regex':{
                    'method':"SET",
                    'options':[""],
                    'default':0,
        ...
        'layout':{
            ...
            'attributes':{}},
        'container':{
            'comment':"Row, column and split layout containers for panel widgets",
            'text':"\n",
            'tail':"\n",
            'sub_elements':[],
            'user_elements':[
            ...
            'attributes':{
                ...
                'enabled':{
                    ...
                    'options':["True", "False"],
                    'default':0,
                    'comment':"If disabled widgets are greyed and not editable"},
                'active':{
                    'method':"SET",
                    'options':["True", "False"],
                    'default':0,
                ...
                'percent':{
                    ...
                    'options':[""],
                    'default':0,
                    'comment':"Split layout percentage (see Blender's API)"}}},
        'widget':{
            ...
            'attributes':{
                ...
                'toggle':{
                    ...
                    'options':["True", "False"],
                    'default':1,
                    'comment':"Draw BOOL as toggle button (see Blender's API)"},
                'trigger':{
                    'method':"SET",
                    'options':[""],
                    'default':0,
    ...
    def _register_panel(self, pipeline, category, panel):
        ...
        def unfold_layout(xmlpath, prefix="", layout="layout", depth=0):
            ...
            for l_name in self.list_elements(xmlpath):
                ...
                l_attrs = {'type':"", 'text':-1, 'box':"", 'align':"", 'percent':"",
                           'enabled':"", 'active':"", 'visible':"", 'prop':"",
                           'expand':"", 'slider':"", 'toggle':"", 'icon':"",
                           'trigger':""}
                
                # Get layout attributes
                for a in self.list_attributes(l_path):
                    if a in l_attrs:
                        l_attrs[a] = self.get_attr(ec, l_path, a, (a != 'trigger'))
                ...
                if l_type == 'ROW' or l_type == 'COLUMN' or l_type == 'SPLIT':
                    ...
                    if l_attrs['enabled']:
                        p.append(prefix + l_var + ".enabled=" + l_attrs['enabled'])
                    if l_attrs['active']:
                        p.append(prefix + l_var + ".active=" + l_attrs['active'])
                    if l_attrs['visible']:
                    ...
                    code.extend(p)
                    code.extend(unfold_layout(l_path, prefix + t, l_var, depth + 1))
                else:
                    if l_type == 'SEP':
                        ...
                        if l_type == 'BUTTON':
                            ...
                            if link_path and self.get_attr(ec, link_path, "link"):
                                l_attrs['icon'] = 'LINKED'
                            else:
        ...
        filters = {'DATA':{'validate':"", 'type':"",
                ...
                'OBJECT':{'validate':"object", 'type':"",
                        'filter':"('MESH', 'CURVE', 'SURFACE', 'META', 'EMPTY')"},
        ...
        for w in [w for w in window_list if w in filters.keys() and register]:
            ...
            # Scene and Render share the same datablock so only process for SCENE
            if scene_only and w == 'RENDER':
                window_types = []
            
            # Generate per window properties
            for wt in window_types:
                prop.append(props[wt] + "." + e_attr + " = bpy.props.BoolProperty(" + \
            ...
            for p in self.list_elements(path + "/properties"):
                ...
                attrs = {'type':"", 'description':"", 'default':"", 'min':"",
                ...
                for a in self.list_attributes(prop_path):
                    if a in attrs:
                        ...
                        if (a == 'description' or a == 'default') and "'" in attrs[a]:
    ...
    def list_elements(self, xmlpath, sort=False, attrs=[]):
        ...
        attrs = build element attribute string [("prefix", 'eval', "postfix"),...]
    ...
    def list_panels(self, category, pipelines=[], type="", window=""):
        ...
        for rmp in pipelines:
            for pan in self.list_elements(rmp + cat):
                ...
                if (not window or window in windows) and (not type or t == type):
    ...
    def get_attr(self, export_object, xmlpath, attr, resolve=True, default=""):
        ...
        export_object = object requesting attr (subclass of ExportContext, None or {})
    ...
    def get_text(self, export_object, xmlpath, resolve=True):
        ...
        export_object = object requesting text (subclass of ExportContext or None)
        ...
        try:
            ...
            if attr != None:
                ...
                if attr:
                    ...
                    value = get_key(k, key)
                # Otherwise return all element attributes and values in dictionary
                else:
                    value = {}
    ...
    def slmeta_to_panel(self, filepath, library, pipeline, panel, write=True, **attribs):
        """Generates an XML shader panel element from a K3D slmeta file.
        
        filepath = path and name of slmeta file to build panel from
        library = RIB path to shader's library if any (prefixed to shader name)
        pipeline = name of pipeline to create panel in
        panel = name of new panel (if already exists will be updated from slmeta)
        write = write changes to pipeline text
        ...
        try:
            ...
            for p in slmeta.findall("shaders/shader/argument"):
                attribs = {'name':"", 'description':"", 'storage_class':"uniform",
                           'type':"string", 'default_value':"", 'array_count':"1",
                           'min':"", 'max':""}
