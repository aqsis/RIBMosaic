...
# #############################################################################
#
# Exporter context module (state info for all objects in pipeline and exporter),
...
class ExportContext():
    ...
    The public members of this interface are exposed to the pipeline editor's UI
    and link system. This facilitates the creation of very complex pipelines. The
    private members of this interface are to aid each respective object in
    accessing and maintaining their own export context.
    """
    ...
    def __init__(self, export_object=None, pointer_datablock=None, pointer_pass=None):
        ...
        if pointer_pass:
            p = pointer_pass
            self.pointer_pass = p
            self.pass_type = getattr(p, "pass_type", self.pass_type)
            self.pass_tile = getattr(p, "pass_tile_index", self.pass_tile)
            self.pass_sequence = getattr(p, "pass_seq_index", self.pass_sequence)
            self.pass_userid = getattr(p, "pass_passid", self.pass_userid)
            self.pass_output = getattr(p, "pass_display_file", self.pass_output)
            self.pass_layer = getattr(p, "pass_layerfilter", self.pass_layer)
            self.pass_multilayer = getattr(p, "pass_multilayer", self.pass_multilayer)
            self.pass_ribstr = getattr(p, "pass_rib_string", self.pass_ribstr)
    ...
    def _resolve_links(self, text, textpath=""):
        ...
        def walk_links(link_list):
            ...
            try:
                ...
                err.ReportError()
                raise rm_error.RibmosaicError("Link errors from \"" + textpath + "\"")
                ...
                if self.pointer_datablock:
                    ...
                    if self.pointer_pass and do_filter:
                        ...
                        if panel_filter:
                            ...
                            try:
                                enabled = eval(self._resolve_links(self, panel_filter))
                            except:
                                raise rm_error.RibmosaicError("Filter expression error", \
                                                              sys.exc_info())
                        else:
    ...
    def ribify_primvar(self, datablock=None, data=None, rmname="", rmtype="", rmclass=""):
        """Export data in geometry datablock to current RIB archive as primvar"""
